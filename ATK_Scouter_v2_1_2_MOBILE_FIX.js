...
/*
 * ATK SCOUTER v2.1.2 - RELEASE NOTES
 * 
 * MAJOR FEATURES:
 * ===============
 * 
 ...
 * 
 * RECENT UPDATES (v2.0.0 → v2.1.2):
 * ==================================
 * 
 * v2.1.2 - Mobile Reload Fix (Current):
 * - Fixed "Save & Reload" button not working on mobile browsers (Torn PDA)
 * - Implemented multiple fallback reload methods for better mobile compatibility
 * - Now uses: window.location.reload(true) → window.location.replace() → window.location.href
...
(() => {
  ...
  const VERSION = '2.1.2-MOBILE-FIX';
...
const LastCheckCacheManager = (() => {
  ...
  function createSettingsUI() {
    ...
    modal.querySelector('#ff-save')?.addEventListener('click', () => {
      ...
      // Mobile-friendly reload: use multiple fallback methods
      // Method 1 works best on desktop, Methods 2-3 work better on mobile browsers
      try {
        // Try hard reload with cache bypass first
        window.location.reload(true);
      } catch (e) {
        console.log('Reload method 1 failed, trying method 2');
        try {
          // Replace current page (works better on mobile)
          window.location.replace(window.location.href);
        } catch (e2) {
          console.log('Reload method 2 failed, trying method 3');
          // Direct href assignment (most compatible)
