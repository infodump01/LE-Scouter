...
(() => {
    ...
    function configureSettings() {
        const lh = prompt('RSI % breakpoint for High→Med (default 100):', 100);
        const hm = prompt('RSI % breakpoint for Med→Low (default 120):', 120);
        const lw = prompt('Life-weight (0–1, default 0.1):', 0.1);
        // store\        rD_setValue('threshold_lowHigh', lh);
        rD_setValue('threshold_highMed', hm);
        rD_setValue('lifeWeight', lw);
    ...
    GM_addStyle(`
        ...
        .ff-settings-btn { cursor:pointer; font-size:1.2em; padding:6px; }
    `);
    ...
    function waitForHeader() {
        ...
        if (!m||!USER_BP) return;
        const h4=document.querySelector('h4'); if(!h4||h4.dataset.ffInjected) return;
        ...
        apiGet(`/user/${id}?selections=personalstats,basic,profile`, od=>{
            ...
            const boost=inv*closeness*parseFloat(rD_getValue('lifeWeight',0.1));
    ...
    function processNode(node){
        ...
        apiGet(`/user/${id}?selections=personalstats,basic,profile`, info=>{
            ...
            arrow.className=`ff-list-arrow ${cls}`+(lp<1?' wounded':''); arrow.style.left=pos;
            node.appendChild(arrow);
        });
    }
    function scan(root=document){ waitForHeader(); root.querySelectorAll('a[href*="profiles.php?XID="]').forEach(l=>{
        let n=l.closest('tr,li,div,td')||l.parentElement; processNode(n);
    }); }

    // ---- Hooks & Button Placement ----
    function injectSettingsButton() {
        // desktop: toolbar > avatar li
        const toolbar = document.querySelector('ul.toolbar.clearfix');
        if (toolbar) {
            const li = document.createElement('li');
            li.className = 'ff-settings-btn';
            li.innerHTML = '<button title="LE Scouter Settings"><span>⚙️</span></button>';
            const avatarLi = toolbar.querySelector('li.avatar');
            avatarLi && avatarLi.insertAdjacentElement('afterend', li);
            li.addEventListener('click', configureSettings);
        }
    }
    window.addEventListener('load', ()=>{ scan(); injectSettingsButton(); });
    window.addEventListener('popstate', ()=>scan());
    new MutationObserver(ms=>ms.forEach(r=>scan(r.target))).observe(document.body,{childList:true,subtree:true});
})();
