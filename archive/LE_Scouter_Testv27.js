// ==UserScript==
// @name         LE Scouter Base v1.2.2 (RSI + Plane Icon Overlay + Live Status)
// @namespace    Violentmonkey Scripts
// @match        https://www.torn.com/*
// @match        https://pda.torn.com/*
// @version      1.2.2-svg-arrow-planeicon-live
...
// @description  RSI arrow and live-updating plane icon (blue/orange) for travel status; tooltip always top layer; all features/settings as before
...
(() => {
  ...
  let ME_STATS = null, ME_DRUGS = null, USER_BP = null, USER_DRUG_DEBUFF = 0, _readyQueue = [];
  // ---- fetch our own stats FIRST, then allow anything else ----
  apiGet('/user/?selections=personalstats,basic', d => { ME_STATS = d; readyCheck(); });
  apiGet('/user/?selections=battlestats',       d => { ME_DRUGS = d; readyCheck(); });
  ...
  function readyCheck() {
    ...
    // Now we're ready; run all waiting injections
    _readyQueue.forEach(f => f());
    _readyQueue = [];
    // normal live hooks:
    injectAll();
  ...
  function injectProfile() {
    ...
    // Don't render RSI banner until we have USER_BP, which is after readyCheck()!
    ...
    // Wait until we're ready (USER_BP loaded)
    if (USER_BP === null) _readyQueue.push(renderProfileBadge);
    else renderProfileBadge();
  }
        ...
        function addPlaneIcon(statusData, triggerEvt) {
          ...
          if (statusData.status && (statusData.status.state === "Traveling" || statusData.status.state === "Abroad")) {
            ...
            plane.className = 'ff-travel-icon' + (statusData.status.state === "Traveling"
              ? ' ff-traveling'
              : ' ff-abroad');
            plane.title = statusData.status.description || statusData.status.state;
            ...
            plane.addEventListener('mousemove', function(ev2){
              showTooltipAt(ev2.pageX, ev2.pageY, plane.title);
            });
            ...
            if (triggerEvt && triggerEvt.pageX) showTooltipAt(triggerEvt.pageX, triggerEvt.pageY, plane.title);
          } else {
            hideTooltip();
          }
        }
  ...
  // If user stats are not ready, all injection is queued until readyCheck
  if (USER_BP === null) _readyQueue.push(injectAll);
  else injectAll();
})();
