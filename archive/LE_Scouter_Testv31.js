...
// Track rows already seen/flashed to prevent double effects
var seenRows = new WeakSet();

function flashAndAttackPatch() {
  // Adjust selector as needed for mobile/desktop! (Based on your screenshots)
  document.querySelectorAll('.rowWrapper___me30X, li.rowWrapper___me30X').forEach(function(row) {
    // -- FLASH: Only flash new seller rows --
    if (!seenRows.has(row)) {
      seenRows.add(row);
    ...
    // --- ATTACK BUTTON ---
    if (row.querySelector('.ff-attack-btn')) return; // Don't double-insert

    // Look for a profile link (player seller)
    ...
    icon.style.filter = 'drop-shadow(0 0 5px #ff1744) drop-shadow(0 0 6px #c62828)';
    icon.style.transition = 'filter 0.2s';
    icon.style.pointerEvents = 'none'; // Make sure button gets click, not img
    ...
    // Insert after the profile link (visible, doesn't break layout)
