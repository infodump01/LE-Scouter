...
(() => {
  ...
  GM_addStyle(`
    ...
    .ff-list-arrow {
      ...
      border-right:20px solid #2e7d32;      /* default “low” color */
      pointer-events:auto;
      z-index:10;
    }
  ...
  function injectProfile() {
    ...
    // — If *any* heading was found (even if dataset.ff was already set), bail out now —
    //    We only ever want exactly one banner next to <h4> on desktop. Do not inject mobile fallback!
    if (heading) {
      return;
    }

    // 2) If *no* desktop heading was found, try to insert under the mobile <h1>…</h1> (once)
    const mobileHeading = document.querySelector('h1');
    if (mobileHeading && !mobileHeading.dataset.ffMobile) {
      ...
      apiGet(`/user/${userId}?selections=personalstats,basic,profile`, o => {
        ...
        `;
        // Insert immediately after <h1> on mobile
        mobileHeading.insertAdjacentElement('afterend', sp);
      });
      return;
    }

    // 3) If we still haven’t injected (no <h4> on desktop, no <h1> on mobile),
    //    fall back to prepending at the very top of the “wrapper” (rare).
