...
// @version      1.2.3
// @description  RSI arrow and live-updating plane icon (blue/orange) for travel status; custom tooltip only (never double), all features/settings as before
      ...
      apiGet(`/user/${userId}?selections=personalstats,basic,profile`, d => {
        ...
        function addPlaneIcon(statusData, triggerEvt) {
          ...
          if (statusData.status && (statusData.status.state === "Traveling" || statusData.status.state === "Abroad")) {
            ...
            plane.addEventListener('click', updatePlaneStatus);
