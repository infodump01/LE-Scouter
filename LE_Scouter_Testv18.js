...
// @description  Two-tab dark GUI; injured+drug indicators; on profiles show Life% & “Last action” (shortened); on list pages inject classic 16×12px RSI arrows into honorWrap with tooltips that follow the cursor’s page coordinates.
// @updateURL    https://raw.githubusercontent.com/infodump01/LE-Scouter/main/LE_Scouter_v1.2.1_life_action_relative_tooltips_cursoranchor_fixed.js
// @downloadURL  https://raw.githubusercontent.com/infodump01/LE-Scouter/main/LE_Scouter_v1.2.1_life_action_relative_tooltips_cursoranchor_fixed.js
...
(() => {
  ...
  // ---- Styles (16×12 arrow; enable pointer‐events; tooltip) ----
  GM_addStyle(`
    /* Score badges */
    .ff-score-badge {
    ...
    /* Triangle arrows (16px tall = 8px top + 8px bottom; 12px wide) */
    .ff-list-arrow {
      position:absolute;
      top:50%;
      ...
      border-right:12px solid #2e7d32; /* default “low” */
      pointer-events:auto; /* Allow hover/tap events */
      z-index:10;
    }
  ...
  // ---- Utility: show/hide a single floating tooltip at cursor page coordinates ----
  function showTooltipAt(pageX, pageY, html) {
    ...
    requestAnimationFrame(() => {
      ...
      // pageX is relative to document; compute left so tooltip is centered horizontally:
      let left = pageX - rect.width/2;
      left = Math.max(4, Math.min(left, document.documentElement.clientWidth - rect.width - 4));
      // place tooltip ~12px above the cursor’s Y
  ...
  // ---- Injection: Profile (shortened last_action.relative) + List (into honorWrap) ----
  function injectProfile() {
    const m = location.href.match(/[?&](?:XID|user2ID)=(\d+)/);
    if (!m || USER_BP === null) return;
    const userId = m[1];

    // Find a heading (h1–h4) containing that user ID:
    ...
    if (!heading || heading.dataset.ff) return;
  ...
  function injectList(root = document) {
    ...
    // On list pages, for every <a href="profiles.php?XID=">…</a>:
    root.querySelectorAll('a[href*="profiles.php?XID="]').forEach(a => {
      ...
      apiGet(`/user/${userId}?selections=personalstats,basic,profile`, d => {
        ...
        // Determine horizontal position percentage across the banner:
        ...
        // 3) Create the arrow inside honorWrap:
        ...
        // 4) Attach tooltip handlers that use event.pageX/pageY:
