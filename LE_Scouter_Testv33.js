...
// ==== BEGIN PDA/PC MARKET PATCH ====

// ... your existing setup and selection logic here ...

function createAttackButton(xid) {
    const btn = document.createElement('button');
    btn.type = 'button';
    btn.title = 'Attack this player';
    btn.className = 'ff-attack-btn-circle';
    ...
    btn.style.boxShadow = '0 0 6px 2px #ff1744, 0 0 12px 4px #ff1744b0';
    btn.style.borderRadius = '50%';
    btn.style.cursor = 'pointer';

    const img = document.createElement('img');
    img.src = 'https://raw.githubusercontent.com/infodump01/LE-Scouter/main/circle.png';
    img.alt = 'Attack';
    img.style.width = '20px';
    img.style.height = '20px';
    img.style.filter = 'drop-shadow(0 0 4px #ff1744)';
    img.draggable = false;

    btn.appendChild(img);

    // Open attack in new window/tab
    btn.onclick = (e) => {
        e.preventDefault();
        window.open(`https://www.torn.com/loader.php?sid=attack&user2ID=${xid}`, '_blank');
        return false;
...
// Example patch logic (pseudo-selector for brevity):
document.querySelectorAll('[class*="rowWrapper"]').forEach(row => {
    // ... your logic to extract XID ...
    const profileLink = row.querySelector('a[href*="profiles.php?XID="]');
    if (!profileLink) return;
    const xid = (profileLink.href.match(/XID=(\d+)/) || [])[1];
    if (!xid) return;

    // Avoid duplicates
    if (row.querySelector('.ff-attack-btn-circle')) return;

    // Insert attack button (position as needed, here before the profile link)
    profileLink.parentElement.insertBefore(createAttackButton(xid), profileLink);
});

// ==== END PDA/PC MARKET PATCH ====

// Don't forget your CSS!
GM_addStyle(`
.ff-attack-btn-circle:hover img { filter: drop-shadow(0 0 6px #ff1744) brightness(1.2); }
`);
