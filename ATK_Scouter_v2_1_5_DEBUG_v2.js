// ==UserScript==
// @name         ATK Scouter v2.1.5-DEBUG-v2
// @namespace    Violentmonkey Scripts
// @match        https://www.torn.com/*
// @match        https://pda.torn.com/*
// @version      2.1.5-DEBUG-v2
// @description  DEBUG v2 - Enhanced debugging with collapsible panel, larger message buffer, and better mobile UX
...
/*
 * ATK SCOUTER v2.1.3 - RELEASE NOTES
 * 
 * MAJOR FEATURES:
 * ===============
 * 
 ...
 * 
 * RECENT UPDATES (v2.0.0 → v2.1.3):
 * ==================================
 * 
 * v2.1.3 - Mobile UI Fix (Current):
...
(() => {
  ...
  // ============================================================================
  // VISUAL DEBUGGING SYSTEM v2 (FOR MOBILE)
  // ============================================================================
  ...
  const VisualDebug = (() => {
    ...
    let showOnlyRecent = false; // Filter to show only last 30 seconds
    ...
    function init() {
      ...
      `;
      header.innerHTML = `
        <span style="flex: 1;">🔍 DEBUG LOG v2 <span id="ff-debug-count" style="opacity: 0.7; font-size: 10px;">(0)</span></span>
        <div style="display: flex; gap: 6px; align-items: center;">
          <button id="ff-debug-filter" style="
            background: #2196F3;
            color: white;
            border: none;
            padding: 3px 10px;
            ...
            font-weight: bold;
          ">Last 30s</button>
          <button id="ff-debug-clear" style="
            background: #f44336;
            color: white;
            border: none;
            padding: 3px 10px;
            ...
            font-weight: bold;
          ">Clear</button>
          <span id="ff-debug-toggle" style="font-size: 16px; width: 20px; text-align: center;">▼</span>
        </div>
      `;
      container.appendChild(header);
      
      const content = document.createElement('div');
      content.id = 'ff-debug-content';
      ...
      messageList.id = 'ff-debug-messages';
      ...
      header.addEventListener('click', (e) => {
        if (e.target.id === 'ff-debug-clear' || e.target.id === 'ff-debug-filter') return;
        isCollapsed = !isCollapsed;
        content.style.display = isCollapsed ? 'none' : 'block';
        document.getElementById('ff-debug-toggle').textContent = isCollapsed ? '▲' : '▼';
      });
      
      // Clear button
      document.getElementById('ff-debug-clear')?.addEventListener('click', (e) => {
        e.stopPropagation();
        const sessionMsg = messages.find(m => m.type === 'session');
        messages.length = 0;
        if (sessionMsg) messages.push(sessionMsg);
        messages.push({
          type: 'warning',
      ...
      // Filter button
      document.getElementById('ff-debug-filter')?.addEventListener('click', (e) => {
        e.stopPropagation();
        showOnlyRecent = !showOnlyRecent;
        const btn = document.getElementById('ff-debug-filter');
        if (showOnlyRecent) {
          btn.style.background = '#FF9800';
          btn.textContent = 'All Messages';
        } else {
          btn.style.background = '#2196F3';
          btn.textContent = 'Last 30s';
        }
        render();
      });
    ...
    function render() {
      if (!container) return;
      const messageList = document.getElementById('ff-debug-messages');
      ...
      if (showOnlyRecent) {
        ...
        displayMessages = messages.filter(m => (m.timestamp || 0) >= cutoff);
      }
      
      messageList.innerHTML = displayMessages.map(msg => {
        const colors = {
          success: '#4CAF50',
          error: '#f44336',
          warning: '#ff9800',
        ...
        const color = colors[msg.type] || '#fff';
        const icons = {
          success: '✓',
          error: '✗',
          warning: '⚠',
        ...
        // Special styling for session marker
        if (msg.type === 'session') {
          return `
            <div style="
              margin: 8px 0;
              ...
              font-weight: bold;
            ">
              <div style="color: ${color}; font-size: 12px;">${icon} ${escapeDebugHtml(msg.text)} ${icon}</div>
              <div style="color: #999; font-size: 9px; margin-top: 4px;">${escapeDebugHtml(msg.detail)}</div>
            </div>
          `;
        }
        ...
        return `
          ...
          ">
            <div style="display: flex; align-items: center; gap: 6px; margin-bottom: 2px;">
              <span style="color: ${color}; font-weight: bold; font-size: 11px;">${icon}</span>
              <span style="color: #888; font-size: 8px; font-family: monospace;">${msg.time}</span>
            </div>
            <div style="color: ${color}; margin-left: 20px; line-height: 1.3;">${escapeDebugHtml(msg.text)}</div>
            ${msg.detail ? `<div style="color: #777; font-size: 9px; margin-left: 20px; margin-top: 2px; font-style: italic;">${escapeDebugHtml(msg.detail)}</div>` : ''}
          </div>
        `;
      }).join('');
      
      // Update count
      const countEl = document.getElementById('ff-debug-count');
      if (countEl) {
        countEl.textContent = `(${displayMessages.length}/${messages.length})`;
      }
    ...
    function log(type, text, detail = null) {
      ...
      // Keep only last 200 messages (increased from 50)
      if (messages.length > 200) {
        // Always keep session marker
        const sessionMarker = messages.find(m => m.type === 'session');
        messages.shift();
        if (sessionMarker && !messages.includes(sessionMarker)) {
          messages.unshift(sessionMarker);
        }
      }
      ...
      if (type === 'error') {
        console.error(prefix, text, detail || '');
      } else if (type === 'warning') {
        console.warn(prefix, text, detail || '');
      } else {
    ...
    // Mark new page load
    function markPageLoad(url) {
      log('info', '━━━ PAGE LOAD ━━━', url || location.href);
    }
    
    // Auto-init when body is ready
    ...
    return {
      success: (text, detail) => log('success', text, detail),
      error: (text, detail) => log('error', text, detail),
      warning: (text, detail) => log('warning', text, detail),
      info: (text, detail) => log('info', text, detail),
      database: (text, detail) => log('database', text, detail),
      markPageLoad,
      init
    };
  })();
...
const LastCheckCacheManager = (() => {
  ...
  async function buildFightIntelPanel(headerEl, opponentId, opponentName, currentRSI) {
    VisualDebug.info('Fight Intel: buildFightIntelPanel called', `OpponentID: ${opponentId}, RSI: ${currentRSI}`);
    
    try {
      // Don't duplicate
      if (document.getElementById('ff-intel-panel')) {
        VisualDebug.warning('Fight Intel: Panel already exists, skipping');
        return;
      }

      VisualDebug.database('Fight Intel: Fetching fight data from DB...');
      ...
      VisualDebug.success('Fight Intel: DB queries complete', `Opponent fights: ${opponentHistory.length}, Win rate data: ${winRateData.totalFights} fights`);
      ...
      VisualDebug.info('Fight Intel: Panel created, preparing to insert');
      
      // Insert after the h4 header (or after RSI+ panel if it exists)
      const rsiPlusPanel = document.getElementById('ff-exp-panel');
      if (rsiPlusPanel) {
        VisualDebug.info('Fight Intel: Inserting after RSI+ panel');
        rsiPlusPanel.after(panel);
      } else {
        VisualDebug.info('Fight Intel: Inserting after header element');
        headerEl.after(panel);
      }
      
      VisualDebug.success('Fight Intel: Panel successfully created and inserted! ✓');

    } catch (e) {
      const errorMsg = e.message || String(e);
      const errorStack = e.stack ? e.stack.substring(0, 200) : 'No stack trace';
      VisualDebug.error('Fight Intel: FAILED to build panel', `Error: ${errorMsg}`);
  ...
  function injectProfile() {
    VisualDebug.info('━━ injectProfile() called ━━', `URL: ${location.href.substring(location.href.indexOf('profiles.php'))}`);
    
    const h = document.querySelector('h4');
    VisualDebug.info('Profile: h4 element check', h ? 'Found h4' : 'No h4 found');
    
    if (!h) {
      VisualDebug.warning('Profile: No h4 element found - aborting');
      return;
    }
    
    if (h.dataset.ff) {
      VisualDebug.warning('Profile: h4 already processed - skipping');
      return;
    }
    
    h.dataset.ff = '1';
    VisualDebug.success('Profile: h4 marked as processed');

    const match = location.href.match(/[?&](?:XID|user2ID)=(\d+)/);
    const userId = match?.[1];
    if (!userId) {
      VisualDebug.error('Profile: No user ID found in URL!', location.href);
      return;
    }

    VisualDebug.success('Profile: Starting injection', `UserID: ${userId}`);
    VisualDebug.info('Profile: Making API call...');

    ApiManager.get(`/user/${userId}?selections=personalstats,basic,profile`, async (o) => {
      try {
        VisualDebug.success('Profile: ✓ API callback executed!', `User: ${o.name || 'Unknown'}`);
        
        // Cache the RSI data immediately
        VisualDebug.database('Profile: Starting RSI cache operation...');
        const cacheStart = Date.now();
        await RSICacheManager.set(userId, o, REFRESH_PRIORITY.PROFILE_VIEW);
        const cacheDuration = Date.now() - cacheStart;
        VisualDebug.success('Profile: ✓ RSI cache complete', `Took ${cacheDuration}ms`);
        ...
        `;
        h.appendChild(sp);
        VisualDebug.success('Profile: RSI badge added');
        ...
        VisualDebug.info('Profile: Building RSI+ panel...');
        buildRSIPlusPanel(h, ME_STATS, o);
        VisualDebug.success('Profile: RSI+ panel built');

        // Build Fight Intelligence panel
        VisualDebug.info('Profile: Calling buildFightIntelPanel...');
        ...
        VisualDebug.error('Profile: Error in callback', errorMsg);
  ...
  const ObserverManager = (() => {
    ...
    function processDOM() {
      // Check if we're on a profile page and if it's a new one
      if (location.href.match(/profiles\.php.*XID=/)) {
        if (lastProfileUrl !== location.href) {
          lastProfileUrl = location.href;
          VisualDebug.markPageLoad(`Profile: ${location.href.match(/XID=(\d+)/)?.[1] || 'unknown'}`);
        }
        VisualDebug.info('ObserverManager: processDOM cycle');
      }
  ...
  function init() {
    ...
    console.log('%c🐛 DEBUG v2 (Collapsible, 200 msg buffer, "Last 30s" filter)', 'color: #FF9800; font-weight: bold');
    console.log('%c═══════════════════════════════════════════════════════════', 'color: #4CAF50; font-weight: bold');
    console.log('Initializing components...');

    // Visual debug startup message
    VisualDebug.success('ATK Scouter DEBUG v2 READY!', 'Panel: collapsible | Buffer: 200 msgs | Filter: Last 30s button');
    ...
    VisualDebug.info('Initialization complete', 'ObserverManager will start after user state init');
  }
