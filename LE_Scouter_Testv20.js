// ==UserScript==
// @name         LE Scouter Base v1.2.1 + Life% & Last Action + Larger Tooltip Arrows (PDA Fixed)
// @namespace    Violentmonkey Scripts
// @match        https://www.torn.com/*
// @match        https://pda.torn.com/*
// @version      1.2.1-life-action-relative-tooltips-larger-pdafix
// @description  Two-tab dark GUI; injured+drug indicators; on profiles show Life% & “Last action” (shortened); on list pages inject larger 20×20px RSI arrows into honorWrap with tooltips that follow the cursor—now also runs on TORN PDA (pda.torn.com) and inserts an RSI banner at the top of mobile profile pages.
// @updateURL    https://raw.githubusercontent.com/infodump01/LE-Scouter/main/LE_Scouter_v1.2.1_life_action_relative_tooltips_larger_pdafix.js
// @downloadURL  https://raw.githubusercontent.com/infodump01/LE-Scouter/main/LE_Scouter_v1.2.1_life_action_relative_tooltips_larger_pdafix.js
...
(() => {
  ...
  // ---- Styles (larger 20×20 arrow; enable pointer-events; tooltip) ----
  GM_addStyle(`
    /* Score badges */
    .ff-score-badge {
  ...
  function injectProfile() {
    ...
    // Try to find a desktop‐style heading (h1–h4) containing “[ID]”.
    ...
    if (heading && !heading.dataset.ff) {
      // Desktop profile found; insert banner after that heading
    ...
    // ---- If we reach here, desktop‐heading wasn’t found →
    // We must be on PDA (mobile) or some layout without <h1–h4>.
    // Insert the banner into a “profile wrapper” at top of the page instead.

    // Look for a generic container that wraps the profile content on mobile.
    // On PDA, there is typically a <div class="bodyWrap__dnkU"> or
    // a <div id="appWrap-main"> or similar. We will attempt several fallbacks.
    ...
    apiGet(`/user/${userId}?selections=personalstats,basic,profile`, o => {
      ...
      // Create the same red banner as on desktop, then prepend to mobileWrapper
      ...
      `;
      // Prepend the banner so it appears at the very top on mobile
  ...
  function injectList(root = document) {
    ...
    // For every <a href="profiles.php?XID=">…</a> on list pages:
    root.querySelectorAll('a[href*="profiles.php?XID="]').forEach(a => {
      ...
      // Ensure honorWrap is position:relative:
      ...
      apiGet(`/user/${userId}?selections=personalstats,basic,profile`, d => {
        ...
        el.addEventListener('mousemove', e => {
          // Keep following the cursor’s page coords:
          showTooltipAt(e.pageX, e.pageY, tooltipHtml);
        });
  ...
  modal.innerHTML = `
    ...
    <div style="text-align:right;">
      ...
      <button class="btn class="ff-modal .btn-cancel"   id="ff-cancel">&#10006; Cancel</button>
    </div>
  `;
