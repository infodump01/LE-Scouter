// ==UserScript==
// @name         FF Scouter Enhanced v10.6-PDAFix
// @namespace    Violentmonkey Scripts
// @match        https://www.torn.com/*
// @version      10.6-pdafix
// @description  RSI badges on profiles and overlays on all account listings; unified for Tampermonkey & Torn PDA with PDA fixes
...
(() => {
    ...
    if (typeof PDA_httpGet === 'function') {
        // Torn PDA: wrap raw body into XHR-like object
    ...
    // ---- Key Management ----
    // Try retrieving stored key
    let API_KEY = rD_getValue('api_key', null);
    // If running in PDA, ensure prompt is available
    const showPrompt = (msg) => {
        if (typeof prompt === 'function') return prompt(msg);
        // fallback for PDA if prompt undefined
        return PDA_showPrompt ? PDA_showPrompt(msg) : window.prompt(msg);
    };
    // Prompt once if missing
    if (!API_KEY) {
        const key = showPrompt('Enter your TORN API Key:');
    ...
    // ---- Poll & Inject Profile Badge ----
    function waitForHeader() {
        const match = location.href.match(/[?&](?:XID|user2ID)=(\d+)/);
        if (!match || !USER_BP) return setTimeout(waitForHeader, 300);
    ...
    // ---- Hooks ----
    window.addEventListener('load', () => scanAndInjectLinks());
    window.addEventListener('popstate', () => scanAndInjectLinks());
    new MutationObserver(records => records.forEach(r => scanAndInjectLinks(r.target)))
