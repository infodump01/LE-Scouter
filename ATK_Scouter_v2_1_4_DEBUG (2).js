...
// @version      2.1.4-DEBUG
// @description  DEBUG VERSION - Advanced RSI combat analysis with visual debugging for mobile troubleshooting
...
(() => {
  ...
  // ============================================================================
  // VISUAL DEBUGGING SYSTEM (FOR MOBILE)
  // ============================================================================
  
  const VisualDebug = (() => {
    const messages = [];
    let container = null;
    let isEnabled = true; // Always enabled in debug version
    ...
    function init() {
      ...
      document.getElementById('ff-debug-clear')?.addEventListener('click', () => {
        messages.length = 0;
        render();
      });
      ...
      messageList.innerHTML = messages.map(msg => {
        ...
        return `
          ...
          ">
            ...
            <span style="color: #ccc; font-size: 9px; margin-left: 4px;">${msg.time}</span>
            <div style="margin-top: 3px; color: ${color};">${escapeDebugHtml(msg.text)}</div>
            ${msg.detail ? `<div style="margin-top: 3px; color: #999; font-size: 10px;">${escapeDebugHtml(msg.detail)}</div>` : ''}
          </div>
        `;
      }).join('');
    ...
    function log(type, text, detail = null) {
      ...
      messages.push({ type, text, detail, time });
...
const LastCheckCacheManager = (() => {
  ...
  function injectProfile() {
    ...
    if (!h || h.dataset.ff) {
      VisualDebug.warning('Profile: No h4 found or already processed');
      return;
    }
    h.dataset.ff = '1';
    ...
    if (!userId) {
      VisualDebug.warning('Profile: No user ID found in URL');
      return;
    }

    VisualDebug.info('Profile: Starting injection', `UserID: ${userId}`);

    ApiManager.get(`/user/${userId}?selections=personalstats,basic,profile`, async (o) => {
      try {
        VisualDebug.success('Profile: API data received', `User: ${o.name || 'Unknown'}`);
        
        // Cache the RSI data immediately
        VisualDebug.database('Profile: Caching RSI data...');
        await RSICacheManager.set(userId, o, REFRESH_PRIORITY.PROFILE_VIEW);
        VisualDebug.success('Profile: RSI cache complete');
  ...
  const ObserverManager = (() => {
    ...
    function processDOM() {
      // Debug: Log when processDOM is called (only on profile pages to reduce noise)
      if (location.href.match(/profiles\.php.*XID=/)) {
        VisualDebug.info('ObserverManager: processDOM running on profile page');
      }
  ...
  function init() {
    ...
    console.log('%c🐛 DEBUG MODE ENABLED (Visual debugging active)', 'color: #FF9800; font-weight: bold');
    console.log('%c═══════════════════════════════════════════════════════════', 'color: #4CAF50; font-weight: bold');
    console.log('Initializing components...');

    // Visual debug startup message
    VisualDebug.success('ATK Scouter DEBUG v' + VERSION + ' STARTED', 'Visual debugging active - check this panel for errors');
