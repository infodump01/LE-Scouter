...
(() => {
  ...
  const BountySniper = (() => {
    ...
    const VISIBILITY_TAKEOVER_TIME = 10000;  // Take leadership after 10 seconds of viewing
    ...
    function setupVisibilityTracking() {
      ...
      visibilityCheckInterval = setInterval(() => {
        if (!isLeader && visibilityStartTime > 0) {
          ...
          if (viewingTime >= VISIBILITY_TAKEOVER_TIME) {
    ...
    function start() {
      ...
      // Setup visibility tracking (auto-takeover after 10s of viewing)
      setupVisibilityTracking();
      
      // Setup watchdog for crash recovery
      setupWatchdog();
      
      // Create panel if needed (has internal duplicate check)
      if (!panelEl) {
        createPanel();
      }
