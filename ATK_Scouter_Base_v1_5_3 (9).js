...
(() => {
  ...
  GM_addStyle(`
    ...
    .ff-sniper-panel {
      ...
      width: 200px;
      max-height: 180px;
      background: linear-gradient(135deg, rgba(25,25,35,0.96) 0%, rgba(15,15,25,0.96) 100%);
  ...
  const BountySniper = (() => {
    // State
    let isEnabled = false;
    let targets = [];  // Current sniper targets
    ...
    const BOUNTY_REFRESH_INTERVAL = 90 * 1000;  // Refresh bounty pages every 90 seconds (was 3 mins)
    const MAX_TARGETS = 5;

    // Check if a page-scraped status matches user's enabled preferences
    // Used as a HINT for prioritization (not a hard filter)
    ...
    // Track when each target was last verified
    ...
    function updateStatus(text) {
      ...
      if (status) {
        ...
        status.innerHTML = `<span class="scanning">${text}</span> | API: ${budget}/min`;
      }
    }
    ...
    function createPanel() {
      ...
      panelEl.className = 'ff-sniper-panel' + (isCollapsed ? ' collapsed' : '');
      panelEl.innerHTML = `
      ...
      const header = panelEl.querySelector('.ff-sniper-header');
    ...
    function start() {
      ...
      console.log(`BountySniper: Started (interval: ${settings.sniperFetchInterval}s, budget: ${settings.sniperApiBudget}/min)`);
    }
