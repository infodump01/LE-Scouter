...
(() => {
  ...
  const BountySniper = (() => {
    ...
    function start() {
      ...
      // OPTION A: Load full state IMMEDIATELY (show targets right away)
      const hasState = loadFullState();
      if (hasState) {
        console.log('BountySniper: Loaded existing state - showing targets immediately');
      }
      
      // Setup cross-tab sync
      setupStorageListener();
      
      // Create panel FIRST (so user sees targets immediately)
      if (!panelEl) {
        createPanel();
      }
      panelEl.style.display = 'block';
      
      // Render any loaded targets immediately
      if (targets.length > 0) {
        renderPanel();
        const progress = cachedCandidates.length > 0 ? Math.round((getCheckedCount() / cachedCandidates.length) * 100) : 0;
        updateStatus(`${targets.length} targets | ${progress}% | Loading...`);
      }
      
      // Mark navigation for other pages
      window.addEventListener('beforeunload', markNavigating);
      window.addEventListener('pagehide', markNavigating);
      
      // OPTION B: Delayed leadership claim
      const recentNav = isRecentNavigation();
      const currentLeader = localStorage.getItem(STORAGE_KEY_LEADER);
      const lastHeartbeat = parseInt(localStorage.getItem(STORAGE_KEY_HEARTBEAT) || '0', 10);
      const heartbeatAge = Date.now() - lastHeartbeat;
      
      // On mobile OR recent navigation: use delayed claim
      // On desktop with no active leader: claim immediately
      const shouldDelayLeadership = isMobilePDA || recentNav || (currentLeader && heartbeatAge < LEADER_TIMEOUT);
      
      if (shouldDelayLeadership) {
        // Start as provisional follower
        isLeader = false;
        updatePanelRole();
        ...
        // Try to receive any existing targets
        receiveTargets();
        
        // Start heartbeat checking (but don't claim leadership yet)
        heartbeatInterval = setInterval(() => {
          if (!isLeader) {
            // Just check for updates, don't try to take over yet
            receiveTargets();
          } else {
            checkLeader();
          }
        ...
        // Desktop with no active leader: immediate start
        console.log('BountySniper: No active leader detected, claiming immediately');
        isProvisionalFollower = false;
        tryBecomeLeader();  // This now starts scanning if we become leader
